...
public class HW02_4108056005_3 extends ThreeSum {
	...
	volatile static int allcount=0;
	boolean hasZero=false;
	
	public static void main(String[] args) {
		HW02_4108056005_3 test = new HW02_4108056005_3();
...
//		System.out.println("minPosIndex = "+minPosIndex);
		...
		if(A.length >= threadNum) {
            Thread[] collaborate = new Thread[threadNum];

            for(int j=0; j<threadNum; j++){
                final int trd = j;
                final int start = posStart;
                final int mpi = minPosIndex;
                int endIndex = 0;
                if(j==7) {
                	endIndex = A.length;
                }
                else {
                	endIndex = div*(trd+1)-1;
                }
                final int end = endIndex;
                collaborate[trd] = new Thread(new Runnable(){
                    public void run() {
                    	HW02_4108056005_2 test = new HW02_4108056005_2();
                    	int count = 0;
                    	// sum of three numbers is zero: O((N^2)/4*log(N/2))
                		for(int i=div*trd; i<end; i++) {
                			for(int j=start; j<A.length; j++) {
                				if(-(A[i]+A[j]) > 0) {
                					if(test.rank(A, -(A[i]+A[j]), j+1, A.length-1) != -1) {
                						count++;
//                						System.out.println(A[i]+", "+A[j]+", "+(-(A[i]+A[j])));
                					...
                					if(test.rank(A, -(A[i]+A[j]), i+1, mpi-1) != -1) {
                						count++;
//                						System.out.println(A[i]+", "+A[j]+", "+(-(A[i]+A[j])));
                					}
                				}
