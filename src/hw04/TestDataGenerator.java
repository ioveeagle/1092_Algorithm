...
public class TestDataGenerator {
	...
	public static void main(String[] args) {
		...
		ra.createData(1000, 1000);	// 第一個參數是陣列長度，第二個測資是數值範圍(-n~n)
		
//		for(int i =0; i<10; i++)
//		{
//			System.out.println((int)(Math.random()*50)*2 + 1);	// 1~99
//			System.out.println((int)(Math.random()*50)*2 + 2);	// 2~100
//		}
//		System.out.println(String.valueOf(false));
	...
	public void createData(int size, int maxlen) 
	{
		...
		try 
		{
			FileWriter fw = new FileWriter("D:\\雲端\\程式檔案\\Eclipse\\file\\1092_Algorithm\\src\\hw04\\testData.txt");
	...
	public String[] readData() {
		...
		try {
			FileReader fr = new FileReader("D:\\雲端\\程式檔案\\Eclipse\\file\\1092_Algorithm\\src\\hw04\\testData.txt");
			BufferedReader bfr = new BufferedReader(fr);
			
			int lines = Integer.parseInt(bfr.readLine());
			A = new String [lines];
			ans = new boolean [lines];
			for(int i=0; i<lines; i++) {
				A[i] = bfr.readLine();
				ans[i] = Boolean.parseBoolean(bfr.readLine());
			}
